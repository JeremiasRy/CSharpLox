...
public class Resolver(Interpreter interpreter) : Expr.IVisitor<ThankYou>, Stmt.IVisitor<ThankYou>
{
    ...
    public ThankYou? VisitClassStmt(Class stmt)
    {
        ...
        Declare(stmt.Name);
        Define(stmt.Name);
        if (stmt.Superclass is not null && stmt.Superclass.Name.Lexeme.Equals(stmt.Name.Lexeme))
        {
        ...
        {
            BeginScope();
            _scopes.Peek().Add("super", new() { Resolved = true });
    ...
    public ThankYou? VisitSuperExpr(Super expr)
    {
        if (_currentClass == ClassType.NONE)
        {
